...
**94** scopes · **1242** members · **16** globals · **36** callbacks · **3** aliases
...
| `is_lethal` | `HERO_LIB.is_lethal(ability: Ability, target: PlayerPawn) -> boolean` | safe | Ability resolved damage vs effective HP. |
| `lethality` | `HERO_LIB.lethality(ability: Ability, target: PlayerPawn) -> table\|nil` | safe | The NUMBERS behind is_lethal: { damage (post-resist, one FULL cast), hp (current+barrier), remaining, lethal, valid }. Lets a charge-up ability invert 'how much charge kills this target'. valid=false => damage unresolvable, do not gate. |
